...
def run_assoc(phenotypes=setup.PATH_WORKING_PHENOTYPES, exclude_chrs=setup.DEFAULT_CHROMOSOME_EXCLUSION_GWAS,
              ...
              binary=False, plink2=True, covariates=None, extra=None, hide_covars=True):
...
def run_gwas(path_phenotypes=setup.PATH_ASIANS_GWAS_PHENOTYPES, path_covariates=setup.PATH_ASIANS_GWAS_COVARIATES,
             use_pheno=None, path_out=setup.PATH_ASIANS_GWAS_RESULTS, hide_covars=True):
    ...
    Output: 3-tuple (stdout, stderr, report_paths)
            report_paths are all the paths of 'xxx/.../xxx.<phenotype>.glm.logistic' files""" 

    ####### Build the list of all reports that will be generated by plink
    # Also take into account if only one phenotype is used (manage path_list and run_assoc accordingly)
    extra = None
    ...
    if use_pheno != None:
        ...
        # 1. Get the list of all phenotypes in the phenotype file (read first line)
        with open(path_phenotypes, 'r') as f:
            first_line = f.readline() # get header line (first one)
        # 2. Extract the phenotypes lists, and exclude FID, IID
        phenos = first_line.split('\t')[2:]
    ####### Create the list of paths
    path_list = [ path_out + '.' + pheno + '.glm.logistic' for pheno in phenos ]

    ####### Run associations with plink, get the output
